...
const INTRO_STORY = [
  ...
  '线虫钻入松树的心脉，把翠绿的针叶一点点啃噬枯萎；',
  '而松墨天牛，则背着这些线虫在林间四处传播瘟疫。',
  '若放任不管，整片松林将枯死殆尽，山脚下的村庄也会遭殃。',
  '你，是最后一个愿意挺身而出的守卫者。',
  '循着守林人的呼唤，你握紧了手中的工具，踏入这片枯黄的山林——',
  '为了这片松林，你必须净化线虫，并亲手击败那传播瘟疫的天牛。',
];
...
const DIALOG = {
  intro: [                    // 开场（stage 0，未接任务）
    ...
    '这些线虫钻进松树的心脉，把树芯一点点啃空，针叶随之枯黄凋零。',
    '更糟的是，松墨天牛会背着线虫，在林间到处传播。',
    '要救这片林子，就必须净化线虫，并从根源上击败传播它们的天牛。',
    '再这样下去，整片山林都要完了，山脚下的村庄也难逃一劫。',
    '我把稿子和召唤器交给你，还给你备了点益生菌。',
    '靠近我领取装备，用益生菌净化线虫，击败天牛，守住这片松林！',
  ],
...
function genWorld() {
  ...
  // 在出生点附近固定生成几个菌落，保证开局随时可补充
  const spTX = WORLD_W >> 1;
  for (let i = 0; i < 6; i++) {
    const ox = ((i * 73) % 300) - 150;   // 出生点左右 150 格内
    const tx = clamp(spTX + ox, 4, WORLD_W - 4);
    const gy = surfaceH[tx];
    if (gy >= WORLD_H) continue;
    probioticPickups.push({
      x: tx * TILE + TILE / 2,
      y: (gy - 1) * TILE - 6,
